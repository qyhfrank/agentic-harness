...
Critique dispatches GPT reviewers, verifies source, owner-adjudicates, and returns a verdict. Do not route critique findings through `receiving-code-review`; that skill is for human/external feedback only.
...
- DO NOT use raw `/fanout` for review-shaped acceptance gates; critique owns verdicts, GSA, source verification, and actionability.
...
- Are existing behavior, interfaces, data contracts, and compatibility preserved unless changed by request?
...
- If a surface is deferred/out of scope, are findings limited to current risks?
...
- Are tests, assertions, logs, metrics, probes, or manual checks sufficient?
...
- If closing a gap requires new heavy browser/DOM harnesses, test frameworks, or validation infrastructure not already used by the repo, prefer `near-blocking` and lighter deterministic proof unless concrete bug evidence exists.
...
Before dispatch, define target/revision, stage (`milestone|final|ad-hoc`), goal, selected goals, non-goals, and boundaries. Smallest goal set for the decision: simplification = Goal 1 + Goal 5; durable-state/permission/concurrency/retry/rollback risk adds Goal 4. Missing material context => `needs_escalation`.
...
- Validation gaps: prefer smallest proof for current slice. If stronger proof needs new heavy infrastructure not already in repo, default to `near-blocking` unless concrete bug evidence exists.
...
If reviewer-pair cardinality is unmet after `/fanout` retry exhausts, return `needs_escalation` with launch-failure reason. Final artifacts record quorum by goal: dispatched, received, missing. Default one full wave per stage; after fixes, rerun narrowly. >10 reviewers or second full wave needs risk reason + narrowed target.
...
Verified findings may still be out of scope. When several reduce to one deferred-surface mismatch, collapse them and prefer trim/split/delete over implementing deferred work.
...
For evidence-only findings, judge whether proposed verification widens repo validation surface. If yes, keep `near-blocking` unless concrete bug evidence exists or heavier verification is already in CI.
...
Storage with harness: one critique artifact owns finding details. Acceptance maps use `F-id -> classification -> actionability -> chosen_action -> evidence/commit`; no Finding/Impact/Trigger/Anchor restates. Manifest/state/context record verdict, quorum, finding IDs/counts, artifact path. Persist base, candidate, diff range/stat, quorum, final findings; no cumulative patch by default. If needed, keep latest pass/latest failure or final review.
