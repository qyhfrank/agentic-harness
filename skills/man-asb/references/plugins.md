...
Directories in `~/.asb/plugins/` are automatically discovered as plugin sources. No config needed. Symlinks supported.
...
Plugin MCP servers follow the plugin lifecycle: enable = available, disable = removed. User-defined servers in `~/.asb/mcp.json` win on ID conflict.

## Component Namespacing

All plugin components are prefixed with the plugin name:
...
| `pluginName@source` | Disambiguate when same name in multiple sources |

### Granular Exclude

Cherry-pick within an enabled plugin:
...
A marketplace is a collection of plugins, identified by `.claude-plugin/marketplace.json`:
...
Marketplace entries have `strict: true` by default. In strict mode, the marketplace entry's metadata (commands, agents, mcpServers paths) takes precedence over the plugin's own plugin.json. Set `strict: false` to let plugin.json be authoritative.
