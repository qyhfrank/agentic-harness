...
| `-B` | `--background` | 否       | subagent 后台运行，主 agent 可同时工作   |
...
- 广泛研究用广度编排，狭窄困难任务用聚焦深度推理。有深度推理 runner（如 `/codex-cli`，通过 `-a codex` 指定）时，优先用于狭窄难题
- 模型选择：prefer opus。不选 sonnet 或 haiku
- 最小有效扇出，但独立任务不设人为低上限
- 非阻塞工作用 background agent；需要结果才能继续时用 foreground
...
3. **启动 agents**：在一个 response 中发起多个并行调用，确保并行执行
   - 默认模式：不设 `run_in_background`，主 agent 等待所有结果返回
   - `--background` 模式：设 `run_in_background=true`，主 agent 可同时执行其他任务

   Dispatch 路由（按 `-a` 类型选择执行路径）：
   - 默认 / `-a opus`：Agent tool（Claude subagent）
   - `-a codex`：先通过 Skill tool 加载 `/codex-cli`，再按其命令模板通过 Bash 调用 `codex exec`。每个 codex agent 是一个独立的 Bash 调用，可并行发起。加载 skill 确保命令格式、sandbox 模式和参数与 `/codex-cli` 保持一致
