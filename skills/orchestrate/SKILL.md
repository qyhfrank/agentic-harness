...
- 平台语法和 child-context 概念边界见 `../using-agents/references/orchestration-architecture.md`。
- 平台能力差异见 `../using-agents/references/orchestration-platform-matrix.md`。
...
2. **前置路由判断**
   - 若任务是 deterministic waiter / scriptable monitor，先按 `../using-agents/references/orchestration-platform-matrix.md` 选择 tool execution surface，而不是直接进入 child-context fan-out。
