...
Rules composed into single `.mdc` with `alwaysApply: true` frontmatter. Agent frontmatter: `name`, `description`, `model` (default `inherit`), `readonly`, `is_background`.
...
Commands output in TOML format.
...
OpenCode uses **singular** directory names (`command`, `agent`, `skill`).
...
Rules use MDC frontmatter. MCP distribution strips `type` field (Trae auto-infers transport). Has install detection.
...
MCP only. Has install detection.

## Custom Targets (DSL)

Define targets in `config.toml` without code:
...
DSL targets register as extension targets, override same-ID builtins. All paths support `~/` expansion.

### Frontmatter Transform Spec

Controls how commands/agents frontmatter is adapted:

| Field      | Type                        | Effect                                     |
|:-----------|:----------------------------|:-------------------------------------------|
| `rename`   | `Record<string, string>`    | Rename frontmatter keys                    |
| `omit`     | `string[]`                  | Remove these keys from output              |
| `include`  | `string[]`                  | Keep only these keys (allowlist)            |
...
Pipeline order: defaults -> join -> omit/include -> rename.
