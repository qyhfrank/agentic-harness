...
All sections optional. Defaults apply when omitted.
...
Profile and project layers use same schema; all fields optional (partial overlay). With `--project`, outputs target project directory.
...
Plugin components auto-expanded into section lists during `asb sync`. No manual plugin entries in `[mcp].enabled` etc.
...
Server fields: `command`, `args`, `env`, `url`, `type` (`stdio | sse | http`). Type auto-inferred from `url`/`command` if omitted. Unknown fields preserved.

Plugin MCP servers (from `.mcp.json` inside plugins) auto-merged when plugin enabled. User-defined servers win on ID conflict.

## Library Entry Formats

### Rules

Markdown files in `~/.asb/rules/`. YAML frontmatter optional.
...
Rules composed into single document per target. `enabled` array order controls composition order.
...
Entire directories copied to each agent's skill location. Deactivated skills cleaned up.
...
`${HOOK_DIR}` resolved to absolute distribution path at sync time.
...
Plugin bundles related components (rules, commands, agents, skills, hooks, MCP) into single enable/disable unit.
...
No manifest required. Bare directory with component subdirectories works. Plugin components namespaced: `pluginName:componentId` (e.g., `context7:docs-researcher`).
...
Custom targets via `[targets.<id>]` in config.toml using DSL. See `references/targets.md` for output paths and DSL spec.
